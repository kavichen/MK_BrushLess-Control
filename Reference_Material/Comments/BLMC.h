...
#define  STEUER_OFF {PORTD &= ~0x38; PWM_OFF; }       //关闭所有的6个场效应管，并终止内部的PWM信号
#define  FETS_OFF {PORTD &= ~0x38; PORTB &= ~0x0E; }    //临时关断；所有的6个场效应管，但内部的PWM信号不停
...
#define SENSE               ((ACSR & 0x10))     //若模拟比较器中断标志位置，则此表达式的值为非零，否则表达式的值为零
#define SENSE_L             (!(ACSR & 0x20))    //MITTEL电压高于被复用的负向输入端，表达式的值为零。反之，则为真。
#define SENSE_H             ((ACSR & 0x20))   //MITTEL电压高于被复用的负向输入端，为真.
